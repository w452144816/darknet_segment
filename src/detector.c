    ...
    float hier_thresh, int dont_show, int ext_output, int save_labels, char *outfile, int letter_box, int benchmark_layers)
{
    ...
    while (1) {
        ...
        double time_loadimg = get_time_point();
        image im = load_image(input, 0, 0, net.c);
        printf("%s: loadimg_moudle_Predicted in %lf milli-seconds.\n", input, ((double)get_time_point() - time_loadimg) / 1000);
        
        image sized;
        double time_resize = get_time_point();
        ...
        float *X = sized.data;
        printf("%s: resize_moudle_Predicted in %lf milli-seconds.\n", input, ((double)get_time_point() - time_resize) / 1000);
        //time= what_time_is_it_now();
	...
	for (int i = 0; i < 40; i++)
	{
        ...
        double time_getbox = get_time_point();
        int nboxes = 0;
        detection *dets = get_network_boxes(&net, im.w, im.h, thresh, hier_thresh, 0, 1, &nboxes, letter_box);
        printf("%s: getbox_moudle_Predicted in %lf milli-seconds.\n", input, ((double)get_time_point() - time_getbox) / 1000);
        double time_nms = get_time_point();
        if (nms) {
            if (l.nms_kind == DEFAULT_NMS) do_nms_sort(dets, nboxes, l.classes, nms);
            else diounms_sort(dets, nboxes, l.classes, nms, l.nms_kind, l.beta_nms);
        }
        printf("%s: nms_moudle_Predicted in %lf milli-seconds.\n", input, ((double)get_time_point() - time_nms) / 1000);
